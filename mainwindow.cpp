...
// chosen as the standard full screen landscape resolution of most Symbian touchscreen devices
...
{
    qDebug("OptionsGamestate::OptionsGamestate");
}
...
void OptionsGamestate::clickedHiscores() {
    ...
    hiscoresDialog.setModal(true); // n.b., need to specify setModal(true) even when using exec(), because of the call to showMaximized()
#if defined(Q_OS_SYMBIAN)
    // n.b., simulator actually better with maximised, but need fullscreen for Symbian!
...
void OptionsGamestate::start() {
    ...
    // Android applications don't quit!
...
void OptionsGamestate::registerScore(int score, int level) {
    ...
    enterHiscoreWindow->setAttribute(Qt::WA_LockLandscapeOrientation); // important for Symbian/Simulator!
...
{
    qDebug("PlayingGamestate::PlayingGamestate");
}
...
bool PlayingGamestate::userQuit() {
    qDebug("PlayingGamestate::userQuit");
...
void PlayingGamestate::mousePressEvent(QMouseEvent *event) {
    ...
    if( m_x >= view->x() && m_x < view->x() + view->width() && m_y >= view->y() && m_y < view->y() + view->height() ) {
        ...
        if( itemPause->contains(itemPause->mapFromScene(QPointF(sm_x, sm_y))) ) {
            paused = !paused;
        }
